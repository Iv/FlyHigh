...
{
	GPS_Init(IFlyHighRC::pInstance()->deviceLine().ascii());
}

void GarminDevice::close()
{
	GPS_Command_Off(IFlyHighRC::pInstance()->deviceLine().ascii());
}
...
bool GarminDevice::flightList(Flight::FlightListType &flightList)
{
	...
	m_flightList.clear();
	nTracks = GPS_Command_Get_Track(IFlyHighRC::pInstance()->deviceLine().ascii(), &trkList);
...
bool GarminDevice::wayPointList(WayPoint::WayPointListType &wpList)
{
	...
	nWps = GPS_Command_Get_Waypoint(IFlyHighRC::pInstance()->deviceLine().ascii(), &wayList);
...
bool GarminDevice::routeList(Route::RouteListType &routeList)
{
	...
	nWps = GPS_Command_Get_Route(IFlyHighRC::pInstance()->deviceLine().ascii(), &wayList);
