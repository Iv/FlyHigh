...
double GPS_Util_Get_Double(const UC *s)
{
	...
	if(!GPS_Little)
	{
		for(i=(int32)sizeof(double)-1;i>-1;--i)
		{
	    *p++ = s[i];
		}
	}
...
int32 GPS_Util_Get_Int(const UC *s)
{
    ...
    if(!GPS_Little)
	for(i=(int32)sizeof(int32)-1;i>-1;--i)
	    *p++ = s[i];
    else
...
uint32 GPS_Util_Get_Uint(const UC *s)
{
    ...
    if(!GPS_Little)
	for(i=(int32)sizeof(uint32)-1;i>-1;--i)
	    *p++ = s[i];
    else
...
float GPS_Util_Get_Float(const UC *s)
{
    ...
    if(!GPS_Little)
	for(i=(int32)sizeof(float)-1;i>-1;--i)
	    *p++ = s[i];
    else
