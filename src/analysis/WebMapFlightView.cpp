	...
	:QMainWindow(parent)
{
	...
	// Map
	m_pWebMapWidget = new WebMapWidget(m_pCentralWidget);
	posX = BoxWidth + Border;
	m_pWebMapWidget->setGeometry(QRect(posX, 0, width() - posX, height() - MenuHeight));

	// Distanz
	m_pDistanceWidget = new QWidget(m_pCentralWidget);
	...
	/* Distanzen */
	m_pGroupBoxDists = new QGroupBox(m_pCentralWidget);

	/* Turnpoints */
	m_pGroupBoxTps = new QGroupBox(m_pCentralWidget);
...
void WebMapFlightView::retranslateUi()
{
	QAction *pAction;
	QString strLabel;
	uint itemNr;
...
void WebMapFlightView::setRouteType(RouteType type)
{
...
void WebMapFlightView::changeRouteType(QAction *pAction)
{
	uint itemNr;
...
void WebMapFlightView::newLatLon(const WebMapTurnPoint *pTp)
{
...
qreal WebMapFlightView::getTpDist(const WebMapTurnPoint *pTp1, const WebMapTurnPoint *pTp2)
{
...
WebMapFlightView::TpListItem WebMapFlightView::createTpListItem(WebMapTurnPoint *pTp)
{
	TpListItem item;
	QLabel *pLabel;
	QFont font;
...
void WebMapFlightView::addTpListItem(TpListItem &item)
{
	uint row;
...
WebMapFlightView::DistListItem WebMapFlightView::createDistListItem(WebMapTurnPoint *pTp1, WebMapTurnPoint *pTp2, WebMapLeg *pWebMapLeg)
{
	DistListItem item;
	QLabel *pLabel;
	QFont font;
...
void WebMapFlightView::addDistListItem(DistListItem &item, uint row)
{
...
void WebMapFlightView::updateDistValues(const WebMapTurnPoint *pTp)
{
	DistList::iterator it;
	WebMapTurnPoint *pTp1;
	WebMapTurnPoint *pTp2;
...
bool WebMapFlightView::isFaiTri() const
{
	qreal minDist;
	qreal dist;
	bool isFai = false;
