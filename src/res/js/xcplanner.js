...
function XCUpdateFlightType() {
	...
	turnpointMarkers = $R(0, flightType.n, true).map(function(i) {
		...
		var marker = new GMarker(defaultTurnpointLatLngs[i], {draggable: DragMarker, icon: icon});
		marker.rev = 0;
		marker.ele = -9999;
...
function XCUpdateRoute() {
	...
	overlays = [];
	overlays.push(new GPolyline(latLngs, flight.color, 2, 1.0));
	...
	if (flight.faiMarkers && $F("faiSectors")) {
		...
		overlays.push(new GPolyline(faiSector([pixels[1], pixels[2], pixels[0]]), COLOR.faiSectors[0], 1, 1.0));
		overlays.push(new GPolyline(faiSector([pixels[2], pixels[0], pixels[1]]), COLOR.faiSectors[1], 1, 1.0));
		overlays.push(new GPolyline(faiSector([pixels[0], pixels[1], pixels[2]]), COLOR.faiSectors[2], 1, 1.0));
/*
...
*/
	...
	if (flight.circuitCenter) {
		if ($F("circuit") == "circle") {
			overlays.push(new GPolyline(circle(flight.circuitCenter.getLatLng(), flight.circuitSize, 32), COLOR.circuit, 1, 1.0));
/*
...
*/
			...
			overlays.push(new GPolyline(sector(flight.circuitCenter.getLatLng(), theta, flight.circuitSize, Math.PI / 2.0, 32), COLOR.circuit, 1, 1.0));
/*
